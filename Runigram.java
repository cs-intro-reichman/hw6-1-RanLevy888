...
public class Runigram {
	...
	public static Color[][] flippedHorizontally(Color[][] image) {
		Color[][] oo = new Color[image.length][image[0].length];
		for(int i = 0 ,  z = image[0].length-1 ; i<image[0].length ; i++ , z-- ){
	...
	public static Color[][] flippedVertically(Color[][] image){
        ...
        for(int j = 0 ; j < image[0].length ; j++)
			for(int i = 0 , z=image.length-1 ;i<image.length; z-- , i++){
		     oo[z][j]=image[i][j];
			}        
	...
	private static Color luminance(Color pixel) {
		int n = (int)(pixel.getRed()*0.299+pixel.getGreen()*0.587+pixel.getBlue()*0.114);
    ...
    for (int i = 0; i < height; i++) {
        for (int j = 0; j < width; j++) {
            ...
            scaledImage[i][j] = image[srcI][srcJ];
        }
    }
...
if (source.length != target.length || source[0].length != target[0].length) {
    ...
    for (int i = 0; i <= n; i++) {
        ...
        Color[][] current = blend(source, target, alpha);
        display(current);
        StdDraw.pause(500);
		}	}
